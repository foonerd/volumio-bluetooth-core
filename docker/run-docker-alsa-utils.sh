...
if [[ -z "$COMPONENT" || -z "$ARCH" ]]; then
  ...
  echo "Example: $0 bluez armhf"
  echo "Example: $0 bluez armhf volumio --verbose"
  exit 1
fi

DOCKERFILE="docker/Dockerfile.alsa.$ARCH"
  ...
  for f in out/$ARCH/*.deb; do
    ...
    case "$ARCH" in
      ...
      armhf)   
        ...
        if [[ "$f" != "$newf" ]]; then
          echo "[VERBOSE] Renaming $f to $newf (ARMv7 VFP3 target)"
        fi
        ;;
      arm64)   
